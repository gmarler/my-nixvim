{ config, lib, ... }:
{
  extraConfigLuaPre = lib.optionalString (lib.elem "mini-hipatterns" config.gmarlervim.text.patterns) ''
  ...
  plugins.mini-hipatterns = lib.mkIf (lib.elem "mini-hipatterns" config.gmarlervim.text.patterns) {
    enable = true;
    settings = {
      highlighters = {
