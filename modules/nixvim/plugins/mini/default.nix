...
let
  hasTreeSitterTextobjects = lib.elem "treesitter-textobjects" config.gmarlervim.editor.textObjects;
in
{
  imports = [
    ./animate.nix
  ...
  plugins = {
    ...
    mini-basics = {
      enable = true;
      settings = {
        # Keep core editing/LSP maps owned by gmarlervim's explicit keymap layer.
