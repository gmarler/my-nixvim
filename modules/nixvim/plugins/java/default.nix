...
in
{
  ...
  plugins.java = {
    ...
    luaConfig.pre = ''
      ...
      function _G.gmarlervim_jdtls.find_root(startpath)
      ...
      function _G.gmarlervim_jdtls.find_root_for_buffer(bufnr)
        local path = vim.api.nvim_buf_get_name(bufnr)
        if path == "" then
          return nil
        end

        return _G.gmarlervim_jdtls.find_root(path)
      end

      function _G.gmarlervim_jdtls.workspace_dir(root, kind)
    ...
    config = {
      ...
      on_new_config.__raw = ''
        function(new_config, root_dir)
          new_config.cmd = {
            "${lib.getExe pkgs.jdt-language-server}",
            "-data",
            _G.gmarlervim_jdtls.workspace_dir(root_dir, "data"),
            "-configuration",
            _G.gmarlervim_jdtls.workspace_dir(root_dir, "config"),
            "-javaagent:${pkgs.lombok}/share/java/lombok.jar",
            "-vmargs",
            "-Xmx4G",
            "-XX:+UseG1GC",
      ...
      root_dir.__raw = ''
        function(bufnr, on_dir)
          local root = _G.gmarlervim_jdtls.find_root_for_buffer(bufnr)
          if root then
            on_dir(root)
          end
        end
