...
let
  javaEnabled = config.gmarlervim.lsp.java == "nvim-java";
in
{
  ...
  plugins.java = {
    ...
    luaConfig.pre = ''
      ...
      _G.gmarlervim_jdtls.status = _G.gmarlervim_jdtls.status or { id = nil, message = nil }

      function _G.gmarlervim_jdtls.status_notify(message)
        if not message or message == "" or message == _G.gmarlervim_jdtls.status.message then
          return
        end

        local id = vim.notify(message, vim.log.levels.INFO, {
          title = "jdtls",
          replace = _G.gmarlervim_jdtls.status.id,
          hide_from_history = true,
          timeout = false,
        })

        _G.gmarlervim_jdtls.status.id = id
        _G.gmarlervim_jdtls.status.message = message
      end
    '';
        ...
        "language/status".__raw = ''
          function(_, data)
            ...
            if message and message ~= "" then
              _G.gmarlervim_jdtls.status_notify(message)
            end
          end
        '';
      };
