...
}:
{
  config = {
    plugins.unified = {
      ...
      enable =
        config.gmarlervim.git.diffViewer == "unified"
        || builtins.elem "unified" config.gmarlervim.git.integrations;
    ...
    keymaps = lib.optionals config.plugins.unified.enable (
      ...
      ++ lib.optionals (config.gmarlervim.git.diffViewer == "unified") [
