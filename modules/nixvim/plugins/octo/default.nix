{ config, lib, ... }:
let
  hasSnacksGh = lib.elem "snacks-gh" config.gmarlervim.git.integrations;
...
{
  plugins.octo = {
    enable = lib.elem "octo" config.gmarlervim.git.integrations;

    lazyLoad.settings.cmd = "Octo";

    settings = {
      picker = lib.mkIf (
        config.gmarlervim.picker.tool != null
      ) pickerByTool.${config.gmarlervim.picker.tool};
      use_local_fs = true;
    };
  };
