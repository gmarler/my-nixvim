...
}:
{
  ...
  plugins = {
    rustaceanvim = {
      ...
      enable = config.gmarlervim.lsp.rust == "rustaceanvim";
      lazyLoad.settings = {
        ft = "rust";
        before.__raw = ''
          function()
  ...
  autoCmd = lib.mkIf config.plugins.rustaceanvim.enable [
    {
      ...
      callback.__raw = ''
        function(args)
          ...
          if vim.b[args.buf].gmarlervim_rustaceanvim_lsp_attach_maps then
            return
          end
          vim.b[args.buf].gmarlervim_rustaceanvim_lsp_attach_maps = true
