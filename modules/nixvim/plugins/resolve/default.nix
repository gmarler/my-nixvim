...
let
  ...
  usesLegacyIntegration = lib.elem "git-conflict" config.gmarlervim.git.integrations;
  isSelected = usesLegacyIntegration || lib.elem "resolve" config.gmarlervim.git.integrations;
  luaConfig = ''
    require('gmarlervim.resolve').setup(
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    warnings = lib.optional usesLegacyIntegration ''
      ...
      Use "resolve" in gmarlervim.git.integrations instead.
    '';

    extraFiles."lua/gmarlervim/resolve.lua".source = ./resolve.lua;
