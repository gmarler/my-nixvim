...
let
  cfg = config.gmarlervim.integrations.accountBacked;
in
{
  options.gmarlervim.integrations.accountBacked = {
  ...
  config = lib.mkMerge [
    (lib.mkIf (!cfg.enable || !cfg.ai.enable) {
      # gmarlervim.ai.plugins drops its account-backed entries through the
      # option's own apply function, which keeps local providers available.
      gmarlervim.ai.chatEnable = lib.mkForce false;
    })
