...
in
{
  options.gmarlervim.profile = lib.mkOption {
    type = lib.types.enum [
      "minimal"
      "basic"
      "standard"
    ...
    description = ''
      ...
      - full: lab profile with every optional and overlapping workflow enabled.
        Applies no overrides: every gmarlervim option keeps its declared
  ...
  config = lib.mkMerge [
    (lib.mkIf (cfg.profile == "minimal") (
      recursiveUpdate minimalProfile {
        gmarlervim.performance.treesitter.whitelistMode = lib.mkDefault true;
      }
    ))
