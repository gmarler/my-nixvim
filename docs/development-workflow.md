...
This page is for working on the my-nixvim repo itself, not for consuming the
built package.
...
2. prefer `gmarlervim.*` option wiring when exposing user-facing behavior
...
- keep edits modular
- prefer semantic `gmarlervim.*` options over one-off plugin hacks
